...
export class HomeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.currentGames = this.isMobile
      ? this.games.filter((game) => game.isOnMobile)
      : this.games.filter((game) => game.isOnComputer);
  }

  async addUsername(username: string) {
    this.firebaseService.updateProfile({ displayName: username });
    if (this.userData.uid)
      return await this.apiClientService
        .findUserByUidAndAddDisplayName(this.userData.uid, username)
        .subscribe((res) => {
